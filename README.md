...
The Gerchberg-Saxton algorithm then begins, in an attempt to retrieve the "secret" phase map. First, ![\Large x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}](https://latex.codecogs.com/svg.latex?&space;A=e^{i&space;\text{rand}(\phi)}) is calculated, where ![](https://latex.codecogs.com/svg.latex?&space;\text{rand}(\phi))  is an array of random phases:
